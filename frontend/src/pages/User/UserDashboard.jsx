...
useEffect(() => {
  ...
  const fetchBugData = async () => {
    try {
      const countsResponse = await bugApi.get("/api/bugs/counts");
      prepareChartData(countsResponse.data);
    } catch (error) {
      console.error("Failed to fetch bug counts", error);
    }

    try {
      const bugsResponse = await bugApi.get("/api/bugs/user");
    ...
    try {
      const priorityCountResponse = await bugApi.get(`/api/bugs/priority-count/${userData.id}`);
      console.log("bugs counts by priority ----->", priorityCountResponse.data);
      setBarChartData(priorityCountResponse.data);
    } catch (error) {
      console.log("failed to fetch user bugs count ", error);
    }

    // try {
    //   const statusCountResponse = await bugApi.get(`/api/bugs/status-counts/${userData.id}`);
    //   console.log("status counts ----->", statusCountResponse.data);
    // } catch (error) {
    //   console.log("failed to fetch user bugs status count ", error);
    // }
